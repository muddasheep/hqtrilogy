...
void EV_FireMP5HQA(struct event_args_s *args);
void EV_FireMP5HQA2(struct event_args_s *args);
...
//	    MP5HQA START
//======================
void EV_FireMP5HQA(event_args_t *args)
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/hq2/hks1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/hq2/hks2.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
		break;
	}
...
// We only predict the animation and sound
// The grenade is still launched from the server.
void EV_FireMP5HQA2(event_args_t *args)
{
	int idx;
	vec3_t origin;
